...
describe("0148-sort-list", () => {
  it("should sort a basic unsorted list", () => {
    expect(listToArray(sortList(buildList([4, 2, 1, 3])))).toEqual([
      1, 2, 3, 4,
    ]);
  });

  it("should sort a list with negative numbers", () => {
    expect(listToArray(sortList(buildList([-1, 5, 3, 4, 0])))).toEqual([
      -1, 0, 3, 4, 5,
    ]);
  });
  ...
  it("should handle already sorted list", () => {
    expect(listToArray(sortList(buildList([1, 2, 3, 4, 5])))).toEqual([
      1, 2, 3, 4, 5,
    ]);
  });

  it("should handle reverse sorted list", () => {
    expect(listToArray(sortList(buildList([5, 4, 3, 2, 1])))).toEqual([
      1, 2, 3, 4, 5,
    ]);
  });

  it("should handle duplicate values", () => {
    expect(listToArray(sortList(buildList([3, 1, 2, 1, 3])))).toEqual([
      1, 1, 2, 3, 3,
    ]);
  });
  ...
  it("should handle list with zeros and negatives", () => {
    expect(listToArray(sortList(buildList([0, -1, 0, 1])))).toEqual([
      -1, 0, 0, 1,
    ]);
  });
});
