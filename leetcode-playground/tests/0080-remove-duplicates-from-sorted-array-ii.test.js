import { removeDuplicates } from '../0080-remove-duplicates-from-sorted-array-ii.js';
...
describe('0080-remove-duplicates-from-sorted-array-ii', () => {
  it("removeDuplicates([1, 1, 2])", () => {
    ...
    if (_ek0 !== undefined) expect(_nums0.slice(0, _ek0).sort((a,b)=>a-b)).toEqual([1, 1, 2]);
  });
  ...
  it("removeDuplicates([0, 0, 1, 1, 1, 2, 2, 3, 3, 4])", () => {
    ...
    if (_ek1 !== undefined) expect(_nums1.slice(0, _ek1).sort((a,b)=>a-b)).toEqual([0, 0, 1, 1, 2, 2, 3, 3, 4]);
  });
  ...
  it("removeDuplicates([1, 1, 1, 1])", () => {
    ...
    if (_ek4 !== undefined) expect(_nums4.slice(0, _ek4).sort((a,b)=>a-b)).toEqual([1, 1]);
  });
});
